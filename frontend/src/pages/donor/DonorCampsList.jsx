...
const CampCard = ({ camp, onRegister, onUnregister, registering }) => {
  ...
  const slotsAvailable = expectedDonors > 0 ? expectedDonors - registeredCount : 0;
  const isFull = slotsAvailable <= 0 && expectedDonors > 0 && !isCompleted && !isCancelled;
  const canRegister = (isUpcoming || isOngoing) && !isFull && !camp.isRegistered;
  const canUnregister = (isUpcoming || isOngoing) && camp.isRegistered;
      ...
      <div className="pt-4 border-t border-gray-100 flex flex-col justify-between items-start gap-3">
        ...
        {!isCompleted && !isCancelled && (
          <div className="pt-4 border-t border-gray-100 w-full mt-3">
            {camp.isRegistered ? (
              <button
                onClick={() => onUnregister(camp._id)}
                disabled={registering || !canUnregister}
                className="w-full flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl border-2 border-red-500 text-red-600 font-semibold hover:bg-red-50 disabled:opacity-50 disabled:cursor-not-allowed transition-all"
              >
                {registering ? <Loader2 className="w-4 h-4 animate-spin" /> : <UserMinus className="w-4 h-4" />}
                {registering ? 'Processing...' : 'Unregister'}
              </button>
            ) : isFull ? (
              <button disabled className="w-full flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-gray-100 text-gray-500 font-semibold cursor-not-allowed">
                <Users className="w-4 h-4" /> Camp Full
              </button>
            ) : (
              <button
                onClick={() => onRegister(camp._id)}
                disabled={registering || !canRegister}
                className="w-full flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-red-600 hover:bg-red-700 text-white font-semibold disabled:opacity-50 disabled:cursor-not-allowed transition-all"
              >
                {registering ? <Loader2 className="w-4 h-4 animate-spin" /> : <UserCheck className="w-4 h-4" />}
                {registering ? 'Processing...' : 'Register to Donate'}
              </button>
            )}
          </div>
        )}
