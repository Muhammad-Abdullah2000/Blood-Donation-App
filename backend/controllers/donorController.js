...
export const getDonorCamps = async (req, res) => {
  try {
    ...
    // Attach isRegistered flag and registeredCount for each camp
    const campsWithStatus = camps.map((camp) => {
      const obj = camp.toObject();
      obj.isRegistered = camp.registeredDonors.some(
        (r) => r.donor.toString() === donorId.toString()
      );
      obj.registeredCount = camp.registeredDonors.length;
      return obj;
    });
...
export const registerForCamp = async (req, res) => {
  try {
    ...
    res.json({ success: true, message: "Registered successfully for the camp" });
  } catch (error) {
...
export const unregisterFromCamp = async (req, res) => {
  try {
    const donorId = req.donor?._id || req.donor?.id;
    const camp = await BloodCamp.findById(req.params.id);
    ...
    const index = camp.registeredDonors.findIndex(
      (r) => r.donor.toString() === donorId.toString()
    );
    if (index === -1)
      return res.status(400).json({ success: false, message: "You are not registered for this camp" });

    camp.registeredDonors.splice(index, 1);
    await camp.save();

    res.json({ success: true, message: "Unregistered from camp successfully" });
  } catch (error) {
